...
클라이언트와 서버 사이에 이루어지는 요청과 응답 과정에서 서버가 수행해야 하는 동작을 지정해서 요청을 보낸다.

### GET

**리소스 조회 메서드**

GET요청시 데이터를 전달하고 싶다면 바디를 사용할 수 있지만 권장하지는 않고 쿼리스트링을 사용한다.

POST로도 조회를 할 수 있지만 GET은 캐싱이 가능하기에 GET을 사용하는 것이 유리하다.

### POST

**전달한 데이터 처리/ 생성 요청 메서드**

메시지 바디에 데이터를 담아 서버에 전달하고 서버는 이 데이터를 처리 할 수 있다.

JSON으로 조회 데이터를 넘겨야 하는 애매한 경우 POST를 사용한다.

### PUT

**리소스를 대체(수정)하는 메서드**

리소스가 있으면 덮어쓰고, 없으면 새롭게 생성된다

즉 기존 데이터가 완전히 새로운 데이터로 대체된다.

때문에 일부분의 수정만 필요하거나 추가를 원한다면 수정 → patch 추가 → post를 사용해야한다.

### PATCH

**리소스 일부 부분을 변경하는 메서드**

put과 달리 일부분의 수정이 가능하기 때문에 새롭게 생성하는 것이 아니라면 patch를 사용한다.

### DELETE

**리소스 제거하는 메서드**

서버에서 해당 데이터베이스의 리소스를 제거한다.